...
public static class HostAppBuilderExtensions
{
    ...
    {
        var azKeyVaultEndpoint = "AZURE_KEY_VAULT_ENDPOINT";
        var keyVaultUri = builder.Configuration[azKeyVaultEndpoint];

        if (!string.IsNullOrWhiteSpace(keyVaultUri))
        {
            builder.Services.AddHealthChecks()
                .AddCheck("KeyVaultConfig",
                    () => !string.IsNullOrWhiteSpace(keyVaultUri)
                        ? HealthCheckResult.Healthy()
                        : HealthCheckResult.Unhealthy($"{azKeyVaultEndpoint} must be specified "), ["live"]);
